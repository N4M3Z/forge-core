...
Machine-local config (`jj config --repo`, `git config`) does not travel with the repo. A fresh clone, another machine, a contributor, or CI gets the committed hook files but not the config that activates them, so a security gate silently fails to fire while still looking installed.
