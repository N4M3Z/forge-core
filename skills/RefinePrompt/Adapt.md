...
Take a source rule or prompt set and adapt it for a target repo that consumes the lib submodule as infrastructure but has its own identity.

### Procedure

1. Read the target repo's `rules/` directory. Classify each file:
    - **Custom**: exists only in the target (for example `IndustrySecrets.md`, `ScanRepository.md`). Never touch these.
...
4. For each rule being ported, apply these transformations:
    ...
    - **Strip forge branding** per ContextualNaming: "forge-cli" becomes "the CLI tool", "forge-core" becomes "the core module", skill names drop the forge prefix.
    - **Add `paths:` frontmatter** when the rule should be scoped (for example `SelfLearning.md` scoped to the target's skill paths).
...
5. Present the adaptation plan as a table before making changes:
    ...
    | KeepChangelog.md   | Missing                           | Add             | Generic, any repo      |
...
- If a rule was renamed in the source (for example `RTK.md` to `UseRTK.md`), rename in the target too.
